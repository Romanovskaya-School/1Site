  ...
  <div class="admin-wrapper">
    ...
    <div id="authScreen" v-if="!isAuthenticated">
      <div class="auth-card">
        ...
        <h2>Вход</h2>
        <p>Войдите с помощью учётной записи администратора Supabase.</p>
        <div class="auth-error" :class="{ show: authError }">{{ authError }}</div>
        <form @submit.prevent="doLogin">
          <div class="auth-input-group">
            <label for="authEmail">Email</label>
            <input type="email" id="authEmail" v-model="email" placeholder="you@example.com" autocomplete="email" required />
          </div>
          <div class="auth-input-group">
        ...
        <div class="page" :class="{ active: activePage === 'applications' }">
          <div class="table-wrap">
            <div class="table-head">
              ...
              <button class="btn btn-outline btn-sm" :disabled="applicationsLoading" @click="refreshApplications">
                {{ applicationsLoading ? 'Обновление…' : '↻ Обновить' }}
              </button>
            </div>
            <div v-if="applicationsError" class="auth-error show" style="margin: 0 20px 16px;">
              Не удалось загрузить заявки: {{ applicationsError }}
            </div>
            <table>
              <thead>
                <tr>
...
const { articles, loadArticles, saveArticle, deleteArticle } = useArticles()
const { siteData, loadSiteData, saveSiteData } = useSiteData()
const { uploadSiteImage } = useSiteMedia()
const {
  applications,
  applicationsError,
  applicationsLoading,
...
const { user, initializeAuth, refreshUser, signIn, signOut } = useSupabaseAuth()
...
const pendingArticleImage = ref(null)
const pendingSiteImages = new Map()
const pendingDiplomaImages = new Map()
...
const isAdmin = (candidate) => candidate?.app_metadata?.role === 'admin'

const rejectNonAdmin = async () => {
  authError.value = 'У этой учётной записи нет роли администратора в Supabase. Выполните SQL-настройку роли и войдите заново.'
  await doLogout()
}
...
const doLogin = async () => {
  ...
  if (error) {
    authError.value = error.message?.includes('Supabase is not configured')
      ? 'Панель не подключена к Supabase. Добавьте переменные окружения на хостинге и пересоберите сайт.'
  ...
  if (result.error) {
    authError.value = 'Эта учётная запись не имеет доступа к панели.'
    await doLogout()
  }
}
...
const refreshApplications = async () => {
  const result = await loadApplications()
  if (result.error) showToast('Не удалось загрузить заявки', 'error')
}
...
const showPage = (page, arg) => {
  ...
  if (page === 'editor') {
    pendingArticleImage.value = null
    if (arg) {
...
const saveTest = async () => {
    ...
    const { error } = await saveSiteData(siteData)
    if (error) {
...
const onImageSelected = (e) => {
  ...
  if (!file) return
  pendingArticleImage.value = file
...
const onSiteImageSelected = (e, field) => {
  const file = e.target.files[0]
  if (!file) return
  pendingSiteImages.set(field, file)
  siteData[field] = URL.createObjectURL(file)
}

const onDiplomaImageSelected = (e, idx) => {
  const file = e.target.files[0]
  if (!file) return
  pendingDiplomaImages.set(idx, file)
  siteData.diplomas[idx].image = URL.createObjectURL(file)
}
...
const saveSettings = async () => {
  for (const [field, file] of pendingSiteImages) {
    const { data, error } = await uploadSiteImage(file, `site-${field}`)
    if (error) {
      showToast(error.message || 'Не удалось загрузить фото', 'error')
      return
    }
    siteData[field] = data.url
  }

  for (const [index, file] of pendingDiplomaImages) {
    const diploma = siteData.diplomas?.[index]
    if (!diploma) continue
    const { data, error } = await uploadSiteImage(file, 'diplomas')
    if (error) {
      showToast(error.message || 'Не удалось загрузить диплом', 'error')
      return
    }
    diploma.image = data.url
  }
  ...
  pendingSiteImages.clear()
  pendingDiplomaImages.clear()
...
const submitArticle = async () => {
  ...
  if (pendingArticleImage.value) {
    const { data, error } = await uploadSiteImage(pendingArticleImage.value, 'articles')
    if (error) {
      showToast(error.message || 'Не удалось загрузить картинку', 'error')
      return
    }
    article.image = data.url
  }
  const { error } = await saveArticle(article)
  if (error) {
...
const confirmDelete = async () => {
  const { error } = await deleteArticle(pendingDeleteId.value)
  if (error) {
...
const processApplication = async (id) => {
  const app = applications.value.find(a => a.id === id)
  if (app) {
    const { error } = await saveApplication({ ...app, status: 'processed' })
    if (error) {
